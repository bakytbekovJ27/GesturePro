...
def put_text(img, text: str, pos, font_size: int = 24,
             color=C_TEXT, bold: bool = False, anchor: str = "lt"):
    """
...
def _put_cv(img, text, pos, size, color, bold, anchor):
    ...
    if anchor in ("ct", "cc"):
        ...
        if anchor == "cc":
            y += th // 2
    cv2.putText(img, text, (x, y + int(size * 0.75)),
...
def draw_gesture_badge(img, text: str, x: int, y: int):
    ...
    cv2.putText(img, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.55, C_TEXT_DIM, 1, cv2.LINE_AA)