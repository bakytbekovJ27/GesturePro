...
const translations = {
  ru: {
    ...
    menu_status: 'Desktop shell теперь создаёт реальный PIN и pairing и в Tauri, и в browser dev режиме. Mock включается только через VITE_DESKTOP_MOCK=1.',
    ...
    load_back: '← Назад',
    load_idle: 'Выберите локальный PDF или дождитесь презентации с телефона по реальному PIN.',
    load_session: 'Сессия подключения',
    load_session_hint: 'В Tauri и browser dev эта сессия создаётся через реальный backend pairing.',
  ...
  en: {
    ...
    menu_title: 'GESTURE SUITE',
    menu_status: 'The desktop shell now creates a real PIN and backend pairing in both Tauri and browser dev mode. Mock stays opt-in through VITE_DESKTOP_MOCK=1.',
    ...
    load_back: '← Back',
    load_idle: 'Pick a local PDF or wait for a phone presentation via a real PIN pairing session.',
    load_session: 'Connection session',
    load_session_hint: 'In both Tauri and browser dev mode, this session comes from real backend pairing.',
