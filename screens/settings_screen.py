...
class SettingsScreen:
    ...
    def _render(self, det_result: dict) -> np.ndarray:
        ...
        self._draw_lang_btn(
            canvas, self._ru_rect, "ru", current_lang, self._btn_ru, pinch
        )
        self._draw_lang_btn(
            canvas, self._en_rect, "en", current_lang, self._btn_en, pinch
        )
    ...
    def _draw_lang_btn(self, canvas, rect, code, current, btn_obj, pinch):
        ...
        draw_rect_rounded(
            canvas, (x, y), (x + bw, y + bh), border, radius=14, thickness=bw_t
        )
    ...
    def _draw_simple_btn(
        self, canvas, rect, label, bg, border, text_col, btn_obj, pinch
    ):
        x, y, bw, bh = rect
        ...
        if hovered and pct > 0:
            ...
            draw_rect_rounded(
                ...
                (x, y),
                (x + fw, y + bh),
                tuple(int(c * 0.25) for c in C_ACCENT2),
                radius=12,
            )
            cv2.addWeighted(ov, 0.5, canvas, 0.5, 0, canvas)
