...
from core.gesture import (
    GestureDetector,
    GESTURE_FIST, GESTURE_POINT, GESTURE_THUMB,
    GESTURE_PALM, GESTURE_PEACE, GESTURE_NONE,
)
from core.renderer import (
    draw_rect_rounded, put_text, draw_cursor, draw_gesture_badge,
    C_PANEL, C_BORDER, C_ACCENT, C_ACCENT2, C_PURPLE,
    C_TEXT, C_TEXT_DIM, C_SUCCESS, C_DANGER,
)
...
COLORS = [
    (0, 80, 255), (0, 200, 60), (0, 180, 255),
    (180, 0, 255), (0, 230, 230), (255, 255, 0), (255, 255, 255),
]
...
class PresentationScreen:
    ...
    def _process_gesture(self, result: dict, detector: GestureDetector):
        ...
        if gesture == GESTURE_POINT:
            ...
            if self._prev_xy:
                cv2.line(self._annotations[self._idx],
                         self._prev_xy, (fx, fy),
                         self._brush_col, self._brush_sz)
            self._prev_xy = (fx, fy)
            return
        ...
        if gesture == GESTURE_THUMB:
            ...
            if self._prev_xy:
                cv2.line(self._annotations[self._idx],
                         self._prev_xy, (ex, ey), (0, 0, 0), self._eraser_sz)
            else:
                cv2.circle(self._annotations[self._idx],
                           (ex, ey), self._eraser_sz // 2, (0, 0, 0), -1)
            self._prev_xy = (ex, ey)
            return
    ...
    def _render(self, cam_frame: np.ndarray) -> np.ndarray:
        ...
        display  = cv2.add(cv2.bitwise_and(slide, slide, mask=mask_inv),
        ...
        if not self._system_on:
            ...
            put_text(display, "HOLD FIST TO ACTIVATE",
                     (W//2, H//2), font_size=38, color=(200, 160, 80),
                     bold=True, anchor="cc")
        ...
        draw_rect_rounded(display,
                          (W-PIP_W-12, H-PIP_H-12),
                          (W-10, H-10), C_BORDER, radius=8)
    ...
    def _draw_hud(self) -> np.ndarray:
        ...
        # Статус
        if self._system_on:
            put_text(hud, Lang.t("hud_active"), (12, 12),
                     font_size=20, color=C_SUCCESS, bold=True, anchor="lt")
        else:
            put_text(hud, Lang.t("hud_paused"), (12, 12),
                     font_size=20, color=(80, 80, 200), bold=True, anchor="lt")

        # Номер слайда
        slide_text = f"{Lang.t('hud_slide')} {self._idx+1} {Lang.t('hud_of')} {self._total}"
        put_text(hud, slide_text, (12, 42),
                 font_size=18, color=C_TEXT_DIM, anchor="lt")
        ...
        put_text(hud, mode_str, (200, 15),
                 font_size=22, color=C_ACCENT2, bold=True, anchor="lt")

        # Цвет кисти
        cv2.rectangle(hud, (200, 48), (260, 75), self._brush_col, -1)
        cv2.rectangle(hud, (200, 48), (260, 75), C_BORDER, 1)
        put_text(hud, f"B:{self._brush_sz}", (268, 48),
                 font_size=16, color=C_TEXT_DIM, anchor="lt")
        ...
        bar_y   = 35
        bar_h   = 6
        cv2.rectangle(hud, (bar_x, bar_y), (bar_x+bar_w, bar_y+bar_h), (40,44,60), -1)
        ...
        put_text(hud, hint, (8, HUD_H - 24),
                 font_size=12, color=(90, 95, 120), anchor="lt")
        put_text(hud, Lang.t("hud_back_menu"), (W - 160, HUD_H - 24),
                 font_size=14, color=C_TEXT_DIM, anchor="lt")
    ...
    def _handle_key(self, key: int) -> str | None:
        if key in (ord('q'), 27):
            ...
            _, mask = cv2.threshold(gray, 1, 255, cv2.THRESH_BINARY)
            mask_inv = cv2.bitwise_not(mask)
            out = cv2.add(cv2.bitwise_and(slide, slide, mask=mask_inv),
                          cv2.bitwise_and(ann, ann, mask=mask))
